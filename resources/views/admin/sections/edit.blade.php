...
<div class="panel registration-form">
    <div class="panel-body">
        <div class="text-center">
            ...
            <h5 class="content-group-lg">Modifier la classe</h5>
        </div>
        <form action={{ route('sections.update',$section->id) }} method="post">
            @csrf          
            @method('put')
            <div class="form-group has-feedback">
                <input type="text" name="name" value="{{ $section->name }}" class="form-control" placeholder="Choisissez Nom du classe">
            ...
            <div class="form-group">
                <select name="level_id" id="level_id" class="form-control" required>
                    <option value="{{ $section->levels->id }}" selected>{{ $section->levels->name }}</option>
                    @foreach ($levels as $level)
                        @if ($level->name != $section->levels->name)
